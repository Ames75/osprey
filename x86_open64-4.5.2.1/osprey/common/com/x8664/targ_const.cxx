...
TCON 
Create_Simd_Const_Float (TYPE_ID ty, std::vector<TCON>& tcons) {
    FmtAssert(ty == MTYPE_V16F4 && (tcons.size() == 4), 
            ("Vector size and type do not match\n"));/*TODO for <>4 sized vectors*/
    TCON c;
    TCON_clear(c);
    TCON_ty(c) = MTYPE_V16I4;
    memcpy(&((c).vals.ival.v3), &(TCON_R4(tcons[3])),4);
    memcpy(&((c).vals.ival.v2), &(TCON_R4(tcons[2])),4);
    memcpy(&((c).vals.ival.v1), &(TCON_R4(tcons[1])),4);
    memcpy(&((c).vals.ival.v0), &(TCON_R4(tcons[0])),4);
    return c;
}

TCON 
Create_Simd_Const_Double (TYPE_ID ty, std::vector<TCON>& tcons) {
    FmtAssert(ty == MTYPE_V16F8 && (tcons.size() == 2), 
            ("Vector size and type do not match\n"));/*TODO for <>2 sized vectors*/
    TCON c;
    TCON_clear(c);
    TCON_ty(c) = MTYPE_V16I4;
    memcpy (&((c).vals.ival.v0), &(TCON_R8(tcons[0])),8);
    memcpy (&((c).vals.ival.v2), &(TCON_R8(tcons[1])), 8);
    return c;
}
#endif
