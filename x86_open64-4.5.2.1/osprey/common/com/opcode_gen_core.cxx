...
struct OPERATOR_info_struct OPERATOR_info[OPERATOR_LAST+1] = {
  ...
  {"OPR_PEXTRW",
   // folding the second argument (kid1) into offset causes premature
   // optimization problems with PRE.  For now, specify which byte (0 .. 7) to
   // extract in kid1
   2 /* nkids */,
   OPERATOR_MAPCAT_OEXP /* mapcat */,
   OPERATOR_PROPERTY_expression |
  ...
  BOOL valid;
    ...
    switch (opr) {
      ...
      case OPR_CONSTRUCTOR:
        valid = (rtype == MTYPE_V16F4 && desc == MTYPE_F4) ||
            (rtype == MTYPE_V16F8 && desc == MTYPE_F8) ||
            (rtype == MTYPE_V32F4 && desc == MTYPE_F4)||
            (rtype == MTYPE_V32I1 && desc == MTYPE_I1)||
            (rtype == MTYPE_V32I8 && desc == MTYPE_I1)||
            (rtype == MTYPE_V32F8 && desc == MTYPE_F8);
        break;
#endif
      case OPR_ADD:
      case OPR_CONST:
