...
O64_Component::O64_Component(O64_COMPONENT component)
    :_invalidCFG(false), _invalidSSA(false)
{
...
O64_Component::~O64_Component()
{
    ...
    // invalidate data structure (e.g., cfg) when they are ready
    if (_invalidCFG)
        _Driver->InvalidateWCFG();
    
    if (_invalidSSA)
        _Driver->InvalidateWSSA();
...
{
    bool dump_ir = false, dump_cfg = false, dump_ssa = false;
    ...
    if (dump_cfg) {
        _Driver->GetCurrentWCFG()->Print(TFile,
            CFG_UTIL::DUMP_CFG|CFG_UTIL::DUMP_EXPR|CFG_UTIL::DUMP_ADOM);
    }

    // dump ir with ssa
    if (dump_ssa) {
        WSSA::fdump_tree_ssa(TFile, _Driver->GetCurrentWN());
    }
}
