...
extern void Remove_Zero_Trip_Loop(WN* wn_loop)
{
  ...
  if (Index_Variable_Live_At_Exit(wn_loop)) {
    LWN_Insert_Block_Before(LWN_Get_Parent(wn_loop), wn_loop, 
                            LWN_Copy_Tree(WN_start(wn_loop)));
    // Since we are anyway deleting the loop right below, the step below is
    // not needed.  It causes the code in LWN_Update_Def_Use_Delete_Tree()
    // to crash because it finds this kid to be NULL.  We make a copy of
    // the start node of the loop while moving it out to prevent the
    // deletion of the loop from deleting the INIT statement too.
    //
    // WN_start(wn_loop) = NULL;
  }
  LWN_Extract_From_Block(wn_loop);
  LWN_Update_Def_Use_Delete_Tree(wn_loop, Du_Mgr);
