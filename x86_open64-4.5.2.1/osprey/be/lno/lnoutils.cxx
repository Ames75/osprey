		...
		BOOL update_pointers, SYMBOL *index_loopno)
{
  ...
  for (INT s=0; s<store_stack->Elements(); s++) {
    ...
    if (use_list && (use_list->Incomplete() || use_list->Len() == 0)) {
      for (INT i=1; i<u; i++) {
  ...
  for (INT l=0; l<load_stack->Elements(); l++) {
    ...
    if (def_list->Incomplete() || def_list->Len() ==0) {
      for (INT i=1; i<u; i++) {
...
// check if any scalar is live-out from the loop.
// Used in loop vectorization having double to
// single precision conversion.

BOOL isLiveOutScalar(WN* scalar_wn, WN* loop){
 return (!All_Uses_Within(scalar_wn, loop));
}
