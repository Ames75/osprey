...
#ifdef TARG_X8664
//-------------- Interface for VN_CONSTRUCTOR_EXPR -----------------
//--------------------------------------------------------------

class VN_CONSTRUCTOR_EXPR : public VN_EXPR
{
private:
   ...
   VN_CONSTRUCTOR_EXPR(OPCODE vcons_opc, 
		   UINT32    num_opnds):
      _vcons_opc(vcons_opc), _num_opnds(num_opnds), _opnd_array(NULL)
   {
      if (num_opnds > 3)
...
public:
   ...
   static VN_CONSTRUCTOR_EXPR *Create(OPCODE vcons_opc, 
				  UINT32    num_opnds);
   ...
   {
      Is_True(i < get_num_opnds(), ("Erroneous VN_CONSTRUCTOR_EXPR::opnd access"));
   ...
   {
      Is_True(i < get_num_opnds(), ("Erroneous VN_CONSTRUCTOR_EXPR::vsym access"));
      return (_opnd_array == NULL? 
   ...
   BOOL is_equal_to(CONST_PTR expr) const
   {
      BOOL truth = (expr != NULL                        &&
		    expr->get_kind() == CONS_OP         &&
		    expr->get_num_opnds() == _num_opnds &&
		    expr->get_opc() == _vcons_opc);
   ...
   void print(FILE *fp = stderr) const
   {
      fprintf(fp, "%s(", OPCODE_name(_vcons_opc));
      for (INT i = 0; i < get_num_opnds(); i++)
      {
	 if (i > 0)
	    fputs(", ", fp);
