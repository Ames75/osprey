...
class ZeroVersionPhi {
private:
      union {
          STMTREP*  _defStmt;
          STMTREP*  _chiDefStmt;
          PHI_NODE* _phiDefNode;
      } _def;
      ZVStmtType _defType;
public:
   ZeroVersionPhi() { _defType = ZV_EMPTY; } 
   void SetDefByPhi(PHI_NODE *phi) { _defType = ZV_DEF_BY_PHI; _def._phiDefNode = phi; }
   void SetDefByChi(STMTREP *chi) { _defType = ZV_DEF_BY_CHI; _def._chiDefStmt = chi; }
   void SetDefByStmt(STMTREP *stmt) { _defType = ZV_DEF_BY_STMT; _def._defStmt = stmt; }

   BOOL IsDefByPhi(void) { return _defType ==  ZV_DEF_BY_PHI; }
   BOOL IsDefByChi(void) { return _defType ==  ZV_DEF_BY_CHI; }
   BOOL IsDefByStmt(void) { return _defType ==  ZV_DEF_BY_STMT; }

   STMTREP *GetDefByChi(void) { return _def._chiDefStmt; }
   STMTREP *GetDefByStmt(void) { return _def._defStmt; }
   PHI_NODE *GetDefByPhi(void) { return _def._phiDefNode; }
   
} ;

typedef class ZeroVersionPhi ZeroVersionPhiType;
...
private:
  ...
  vector<ZeroVersionPhiType> defStmtVec;
...
public:
	   ...
	   { vec = (PHI_ELEM*) CXX_NEW_ARRAY(PHI_ELEM, (in_degree + 1), pool);
             defStmtVec.reserve(in_degree + 1);
	     size = in_degree; count = 0; _bb = bb; 
	     _flags = PNF_NONE; }
        ~PHI_NODE(void)                  
         { /*CXX_DELETE_ARRAY(vec, Opt_default_pool);*/ 
           defStmtVec.clear(); }
  
  void SetDef(INT32 i, ZeroVersionPhiType& zvP) {

      if ( zvP.IsDefByPhi() )
          SetDefByPhi(i,zvP.GetDefByPhi() );
      else if (  zvP.IsDefByChi() )
          SetDefByChi(i,zvP.GetDefByChi() );
      else if ( zvP.IsDefByStmt() )
          SetDefByStmt(i,zvP.GetDefByStmt() );
  }

  void SetDefByChi(INT32 i, STMTREP *chi) {
      defStmtVec[i].SetDefByChi(chi);
  }
  void SetDefByPhi(INT32 i, PHI_NODE *node) {
      defStmtVec[i].SetDefByPhi(node);
  }
  void SetDefByStmt(INT32 i, STMTREP *stmt) {
      defStmtVec[i].SetDefByStmt(stmt);
  }
  BOOL IsDefByPhi(INT32 i) { return  ( defStmtVec[i].IsDefByPhi() ) ; }
  BOOL IsDefByChi(INT32 i) { return  ( defStmtVec[i].IsDefByChi() ) ; }
  BOOL IsDefByStmt(INT32 i) { return  ( defStmtVec[i].IsDefByStmt() ) ; }

  STMTREP *GetDefByStmt(INT32 i) { return defStmtVec[i].GetDefByStmt(); }
  STMTREP *GetDefByChi(INT32 i) { return defStmtVec[i].GetDefByChi(); }
  PHI_NODE *GetDefByPhi(INT32 i) { return defStmtVec[i].GetDefByPhi(); }
