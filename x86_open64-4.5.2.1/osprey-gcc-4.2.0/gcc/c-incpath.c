...
static void add_standard_paths (const char *, const char *, const char *, int, int);
...
add_standard_paths (const char *sysroot, const char *iprefix,
		    const char *imultilib, int cxx_stdinc, int preprocessed)
{
  const struct default_include *p;
  int relocated = cpp_relocated();
  size_t len;

  /* Note that the Open64 driver will invoke the GCC driver when
     preprocessing the file (and thus cpp_relocated() will detect that
     the install tree has been relocated).
     However the Open64 driver directly invokes cc1 to do the real
     front-end processing, and in this case cpp_relocated will not
     detect that the install tree has been relocated.
     Fortunately when the Open64 driver invokes cc1, it always supplies
     a preprocessed file as input and thus it is safe to assume that
     install tree has been relocated since include paths since the
     include paths will not be referenced. */
  if (preprocessed)
      relocated = 1;
			 ...
			 int stdinc, int cxx_stdinc, int verbose)
{
  ...
  if (stdinc)
    add_standard_paths (sysroot, iprefix, imultilib, cxx_stdinc, cpp_opts->preprocessed);
